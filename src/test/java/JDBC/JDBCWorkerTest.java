...
public class JDBCWorkerTest {
     ...
     */
    @Test
    public void writeAndReadRepo() throws SQLException {
        fillInMobileConnection();
        fillInWiredInternet();
        JDBCWorker jdbcWorker=new JDBCWorker(JDBCWorker.defaultUrl,JDBCWorker.defaultUser,JDBCWorker.defaultPassword);
        jdbcWorker.rollback();
        jdbcWorker.writeRepo(mobileConnectionRepository,1);
        jdbcWorker.writeRepo(wiredInternetRepository,2);
        Repository<MobileConnection> mobile=jdbcWorker.readRepo(1,"MobileConnection");
        Repository<WiredInternet> wired=jdbcWorker.readRepo(2,"WiredInternet");
        checkingTwoRepo(wiredInternetRepository,wired);
        checkingTwoRepo(mobileConnectionRepository,mobile);
        jdbcWorker.rollback();
    }
}